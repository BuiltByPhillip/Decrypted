    ...
    return (
        <div className={`flex flex-col items-center p-8 gap-3 ${className}`}>
            ...
            <span className="text-4xl font-bold text-gray">
                {definitions && description ? substituteRolesInString(description, definitions) : description}
            </span>
            
            {/* Prompt*/}
            {prompt && <span className="text-xl text-gray/70">{definitions ? substituteRolesInString(prompt, definitions) : prompt}</span>}
        </div>
    );
}